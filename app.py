...
# --- 3. [개선] 초정밀 텍스트 및 수식 교정 엔진 ---
def polish_output(text):
    if not text: return ""
    # 1. 지문 내 모든 메타데이터 및 불필요 문구 삭제
    text = re.sub(r'^(과목|단원|배점|유형|난이도|수학\s?[I|II|1|2]):.*?\n', '', text, flags=re.MULTILINE | re.IGNORECASE)
    text = re.sub(r'^\[.*?점\]\s*', '', text)
    text = re.sub(r'이 문제는.*?문제입니다\.?', '', text)
    
    # 2. 백슬래시 누락 보정 (핵심 수식 기호 보호)
    math_words = ['frac', 'theta', 'pi', 'sqrt', 'log', 'lim', 'to', 'infty', 'sin', 'cos', 'tan', 'sum', 'int', 'alpha', 'beta', 'gamma', 'delta']
    ...
    <head>
        ...
        <style>
            ...
            * {{ font-family: 'Nanum Myeongjo', serif !important; }}
            ...
            .section-title {{ font-size: 14pt; font-weight: 800; border: 2px solid #000; display: inline-block; padding: 3px 15px; margin: 20px 0; }}
            ...
            .options-container {{ margin-top: 25px; display: flex; flex-wrap: wrap; gap: 12px 5px; font-size: 10.5pt; }}
...
async def generate_batch_ai(q_info, size=3):
    ...
    prompt = f"과목:{q_info['sub']} | 배점:{q_info['score']} | 유형:{q_info['type']}\n[지침] 수식은 $ $ 필수. 5지선다는 options 배열에 5개. 메타데이터(과목명, 단원 등) 절대 노출 금지. JSON 배열 {size}개 생성: [{{ \"question\": \"...\", \"options\": [...], \"solution\": \"...\" }}]"
    try:
...
async def get_safe_q(q_info, used_ids, used_batch_ids):
    ...
    return {"num": q_info['num'], "question": "지연 발생", "options": [], "solution": "오류", "cat": q_info['cat']}
...
async def run_orchestrator(sub_choice, num_choice, score_choice=None):
    ...
    for i in range(0, len(blueprint), chunk_size):
        ...
        status.text(f"⏳ {i+1}번 ~ {min(i+chunk_size, 30)}번 문항 작업 중...")
    ...
    for cat_name in ["공통", "선택", "맞춤"]:
        ...
        for item in cat_qs:
            ...
            q_rendered.append(f"<div class='question-box'><span class='q-num'>{item.get('num')}</span> {q_text} <b>[{item.get('score',3)}점]</b>{opt_html}</div>")
            s_html += f"<div class='sol-item'><b>{item.get('num')}번:</b> {polish_output(item.get('solution',''))}</div>"
        
        # 페이지 조판 (한 페이지당 약 6~8문항 배치)
        for j in range(0, len(q_rendered), 8):
            page_chunk = "".join(q_rendered[j:j+8])
            section_box = f"<div class='section-title'>{label}</div>" if j == 0 else ""
            p_html += f"<div class='paper'><div class='header'><h1>2026 수능 모의평가 (수학 영역)</h1></div>{section_box}<div class='question-grid'>{page_chunk}</div></div>"
...
with st.sidebar:
    ...
    if st.session_state.verified:
        ...
        score = int(st.selectbox("배점 선택", ["2", "3", "4"])) if mode == "맞춤 문항" else None
        btn = st.button("🚀 모의고사 발간 시작", use_container_width=True)
        with DB_LOCK: st.caption(f"🗄️ DB 축적량: {len(bank_db)}")

if st.session_state.verified and btn:
    with st.spinner("수능 표준 규격 조판 및 수식 정밀 교정 중..."):
