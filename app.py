...
def generate_exam(subject, difficulty, count, email):
    # [에러 해결] models/ 접두사를 제거하여 404 에러를 방지합니다.
    ...
    for i in range(1, count + 1):
        ...
        try:
            ...
            if "---SPLIT---" in raw_text:
                ...
                q_content = parts[0].replace("[문항]", "").strip()
                s_content = parts[1].replace("[해설]", "").strip()
                q_html_list.append(q_content)
                s_html_list.append(s_content)
            else:
                # 구분자가 없을 경우 전체를 문제로 처리
            ...
            st.error(f"{i}번 생성 중 에러 발생: {e}")
            continue
            
    status_text.success(f"✅ {count}개 문항 발간 완료!")
    percent_text.empty()
