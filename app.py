...
ADMIN_EMAIL = "관리자_본인_이메일@example.com" # 여기에 본인 이메일을 쓰세요 (인증 후 무제한)

# --- 2. 이메일 및 인증 로직 ---
def send_verification_email(receiver_email, code):
    """사용자에게 6자리 인증번호를 SMTP로 발송"""
    try:
        msg = MIMEMultipart()
        msg['From'] = SENDER_EMAIL
        msg['To'] = receiver_email
        msg['Subject'] = "[Premium 수능수학] 인증번호 확인"
    ...
    if can_use:
        ...
        if st.button("🚀 시험지 발간"):
            ...
            if email_input != ADMIN_EMAIL:
                db.table('users').update({'count': db.table('users').get(User.email == email_input)['count'] + 1}, User.email == email_input)
    else: st.error("횟수 초과!")
