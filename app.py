...
st.title("⚡ AI 수능 모의고사 생성기 (최종 최적화 버전)")

# 1. 디자인 템플릿 (250px 여백 및 KeyError 방지용 변수 통일)
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="ko">
<head>
    ...
    <style>
        ...
        .question {{ 
            margin-bottom: 250px; /* 요청하신 넉넉한 풀이 공간 복구 */
            position: relative; 
            padding-left: 25px; 
...
async def fetch_chunk(model, start_num, end_num, subject):
    ...
    try:
        ...
        return clean_text.replace('\\\\', '\\').replace('\\W', '\\')
    except: 
        return ""
...
if "GEMINI_API_KEY" in st.secrets:
    ...
    with st.sidebar:
        ...
        st.divider()
        st.info("⚡ 새로운 API 키와 고속 모드가 적용되었습니다.")

    if st.sidebar.button("🚀 초고속 모의고사 발간"):
        with st.status(f"⏳ {num_q}문항을 동시에 제작 중입니다...") as status:
            try:
                # 고속 생성 함수 호출
                full_content = asyncio.run(generate_fast(model, subject_opt, num_q))
                
                if full_content:
                    # [해결] KeyError 방지: 템플릿의 {content}와 이름을 일치시킴
                    final_html = HTML_TEMPLATE.format(subject=subject_opt, content=full_content)
                    
                    st.success("✅ 발간 완료!")
                    st.components.v1.html(final_html, height=1000, scrolling=True)
                else:
                    st.error("❌ 생성 실패. API 한도(Quota)를 확인하거나 잠시 후 시도하세요.")
