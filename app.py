...
def get_pro_guide(score):
    if score == 2:
        return "[2점 절대 엄수] 무조건 1분 컷 단순 연산. 복잡한 추론/도형/급수 절대 금지."
    elif score == 3:
        return "[3점 응용] 개념 2개 결합 또는 교과서 유제 수준."
    else:
        return "[4점 킬러] 복합 추론, 케이스 분류 필수. 변별력 있는 문항."

# --- 5. HTML/CSS 템플릿 (선지 5열 정렬 및 SVG) ---
...
[지시] 1.한국어 2.{guide} 3.그림 필요시 설명하지 말고 직접 <svg> 태그 코드를 작성해 svg_draw 필드에 주입 4.수식 $$ 필수 5.JSON {size}개 생성:
        ...
        with DB_LOCK:
            for q in batch:
                if q.get('topic') and q.get('question') and q.get('solution'):
                    if expected_type != '객관식' or (isinstance(q.get('options', []), list) and len(q.get('options', [])) == 5):
...
async def get_safe_q(q_info, used_ids, topic_counts, total_num):
    ...
    return {"num": q_info['num'], "score": q_info['score'], "question": "시스템 부하로 인해 예비 문항이 로딩되었습니다. ($2^3 \\times 2^2$ 의 값을 구하시오.)", "options": ["16", "32", "64", "128", "256"], "solution": "32", "source": "SAFE", "svg_draw": None}
...
async def run_orchestrator(sub_choice, num_choice, score_choice=None):
    ...
    if num_choice == 30:
        for i in range(1, 16):
            ...
            blueprint.append({"num": i, "sub": sub, "topic": m1_topics[(i//2)%3] if sub=="수학 I" else m2_topics[(i//2)%3], "score": 2 if i<=3 else 4 if i>8 else 3})
        for i in range(16, 23):
            sub = "수학 II" if i % 2 == 0 else "수학 I"
            blueprint.append({"num": i, "sub": sub, "topic": m2_topics[i%3] if sub=="수학 II" else m1_topics[i%3], "score": 4 if i>20 else 3})
    ...
    while True:
        try:
            ...
            if cur_len < 10000:
                ...
                prompt = f"과목:{sub} | 배점:{score} | [지시] 기준 문항 1개와 변형 3개를 JSON으로 생성. 수식 $$, 그림 필요시 <svg> 필수."
    ...
    if not st.session_state.verified:
        ...
        if email_in == ADMIN_EMAIL:
            if st.button("관리자 로그인"):
                st.session_state.verified, st.session_state.user_email = True, ADMIN_EMAIL
                st.rerun()
        else:
            if st.button("인증번호 발송"):
                ...
                if send_verification_email(email_in, code):
                    ...
                    st.success("인증번호가 이메일로 발송되었습니다!")
                else:
                    st.error("메일 발송에 실패했습니다. (이메일 및 앱 비밀번호 확인 필요)")
                    
            if st.session_state.mail_sent:
                c_in = st.text_input("6자리 인증번호 입력")
                if st.button("인증 확인"):
                    if c_in == st.session_state.auth_code:
                        st.session_state.verified, st.session_state.user_email = True, st.session_state.temp_email
                        st.session_state.mail_sent = False
                        st.rerun()
                    else:
        ...
        with DB_LOCK: st.caption(f"🗄️ 누적 무결점 DB: {len(bank_db)} / 10000")

if st.session_state.verified and btn:
    with st.spinner("AI가 SVG 도면을 설계하고 수능 비율에 맞춰 조판 중입니다..."):
        try:
