...
<body>
    <div class="no-print"><button class="btn-download" onclick="downloadPDF()">📥 PDF 파일 저장</button></div>
    <div class="paper">
...
if "API_KEYS" in st.secrets:
    ...
    st.error("API_KEYS가 설정되지 않았습니다.")
    st.stop()

# 3. 고속 병렬 생성 로직 (수식 깨짐 방지 처리 강화)
async def fetch_chunk(start, end, subject, diff):
    current_key = next(key_cycle)
    genai.configure(api_key=current_key)
    model = genai.GenerativeModel('models/gemini-2.5-flash')
    
    # AI에게 백슬래시를 정확히 출력하도록 지시
    prompt = f"""
    인사말 없이 HTML만 출력. 수능 수학 {subject} {start}~{end}번 문항({end-start+1}개) 제작. 난이도: {diff}.
    수식은 반드시 $ 기호를 사용한 LaTeX로 작성하되, 백슬래시(\)가 누락되지 않도록 주의해.
    예: \\\\frac{{1}}{{2}}, \\\\sqrt{{3}}, \\\\lim_{{n \\\\to \\\\infty}}
...
async def generate_exam(subject, total, diff):
    chunk_size = 2 if diff == "킬러" else 5
    tasks = [fetch_chunk(i, min(i+chunk_size-1, total), subject, diff) 
...
if st.sidebar.button("🚀 초고속 발간"):
    with st.status("⏳ 수식 렌더링 최적화 중...") as status:
        qs, sols = asyncio.run(generate_exam(sub_opt, num_opt, diff_opt))
        if qs:
            final_html = HTML_TEMPLATE.format(subject=sub_opt, questions=qs, solutions=sols)
            st.components.v1.html(final_html, height=1200, scrolling=True)
            st.success("✅ 발간 완료!")
        status.update(label="발간 작업 완료", state="complete")
