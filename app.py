...
# --- 3. [완벽] 정제 엔진 (image_13319d 메타데이터 제거) ---
def polish_output(text):
    if not text: return ""
    # 1. 문제 시작 부분의 모든 메타데이터 패턴 제거
...
# --- 5. HTML/CSS 템플릿 (1페이지 2문제 고정 + 디자인 정제) ---
    ...
    <head>
        ...
        <style>
            ...
            * {{ font-family: 'Nanum Myeongjo', serif !important; }}
            ...
            .header {{ text-align: center; border-bottom: 2.5px solid #000; margin-bottom: 30px; padding-bottom: 5px; }}
            .cat-header {{ font-size: 13pt; font-weight: 800; border-bottom: 1.5px solid #000; display: block; padding: 5px 0; margin-bottom: 25px; }}
            .question-grid {{ display: grid; grid-template-columns: 1fr 1fr; column-gap: 55px; height: 215mm; position: relative; }}
            .question-grid::after {{ content: ""; position: absolute; left: 50%; top: 0; bottom: 0; width: 1px; background-color: #ddd; }}
            .question-box {{ position: relative; line-height: 2.3; font-size: 11pt; padding-left: 30px; margin-bottom: 60px; text-align: justify; }}
...
async def generate_batch_ai(q_info, size=3):
    ...
    prompt = f"과목:{q_info['sub']} | 배점:{q_info['score']} | 유형:{q_info['type']}\n[지시] 수식 $ $ 필수. 객관식은 options 5개. 메타데이터(과목명 등) 절대 금지. JSON 배열 {size}개: [{{ \"question\": \"...\", \"options\": [...], \"solution\": \"...\" }}]"
    try:
...
async def run_orchestrator(sub_choice, num_choice, score_choice=None):
    ...
    for i in range(0, len(results), 2):
        ...
        q_html_content = ""
        for item in pair:
            # 영역 구분 제목 (image_1338a4 박스 해결)
            header_box = ""
            if item['num'] == 1 and not common_rendered:
                header_box = "<div class='cat-header'>■ 공통과목 (수학 I, 수학 II)</div>"
                common_rendered = True
            elif item['num'] == 23 and not select_rendered:
                header_box = f"<div class='cat-header'>■ 선택과목 ({sub_choice})</div>"
            ...
            q_html_content += f"{header_box}<div class='question-box'><span class='q-num'>{item['num']}</span> {q_text} <b>[{item['score']}점]</b>{opt_html}</div>"
            s_html += f"<div class='sol-item'><b>{item['num']}번:</b> {polish_output(item.get('solution',''))}</div>"
        
        p_html += f"<div class='paper'><div class='header'><h1>2026 수능 모의평가 (수학 영역)</h1></div><div class='question-grid'>{q_html_content}</div></div>"
...
with st.sidebar:
    ...
    if st.session_state.v:
        ...
        score = int(st.selectbox("배점 설정", ["2", "3", "4"])) if mode == "맞춤 문항" else None
...
if st.session_state.v and btn:
    with st.spinner("수능 표준 규격 조판 엔진 가동 중..."):
